function nextstate = one_field_day(state,date,a)% bee model in the field season 


agemax=a; % indexing in matlab starts at 1, so add an extra day
...
if  date > startdate %&& date < enddate
    ...
    st2= (0.85)^(1/5); %---survivorship for larval stage
    st3= (0.86)^(1/12);
    st4= (0.85)^(1/12); % 0.99-85%--survivorship for nurse bee stage
    st5=(0.85)^(1/10); % 0.96-88.6%--survivorship for house bee stage
    st6=(0.78)^(1/8); % 78.5%--survivorship for forager bee stage
    qh=0.5 ; %0.06;
...
survivorship(4:8) = (st2*min(1,max(0,1-0.15*(1-Indexpollen*IndexNursing))))^(1/5); %  LARVA 
...
survivorship(21:32)= (1-u)*(st4*max(0,min(1,1-IndexNursing)))^(1/12) ; %= (1-u)*st4^(1/16);
% = (1-u)*(st4*max(0,min(1,1-IndexNursing)))^(1/16) ;
% = (1-u)*max(0,(1-(1-st4)*IndexNurseload))^(1/16);

%survivorship of HOUSE bee
survivorship(33:42)= st5^(1/10);%(st5*min(1,1-Indexhoney))^(1/6);

survivorship(43:agemax)= (1-v)*st6^(1/8); % v is reversed probability of the forager bee stage to revert back to in-hive nurse bees. 
...
%Everything here just relates to storage and matrix multiplication.
        ...
        B(43,21:32)=u*ones(1,12);

        C=zeros(agemax);
        C(21,32:agemax)= v*ones(1,19); % the retarded development of forager bees 
        transit=A+B+C; 
...
notsurvive = ones(1,20);
for k = 1:20
    notsurvive(k) = 1-survivorship(k);
end

scavanged = notsurvive*Nt(1:20); %this was the line that was generating an error before- causing Vt to be a matrix
